...
function pdf_build_address($outputlangs,$sourcecompany,$targetcompany='',$targetcontact='',$usecontact=0,$mode='source',$object=null)
{
	...
	if (is_object($hookmanager))
	{
		...
		//$stringaddress.=$conf->global->MAIN_INFO_TVAINTRA;
	}
	if (empty($reshook))
	{
    	if ($mode == 'source')
    	{
    		...
    		{
					//tax
					if ($sourcecompany->url) $stringaddress .= ($stringaddress ? "\n" : '' ).$outputlangs->transnoentities("Tax ID").": ".$outputlangs->convToOutputCharset($conf->global->MAIN_INFO_TVAINTRA);
    			// Phone
